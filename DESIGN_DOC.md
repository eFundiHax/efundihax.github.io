...
4. **Master Doc Splitting (PENDING, NotebookLM limit):** A single master doc must stay under NotebookLM's **per-source limit of 500,000 words / 200 MB** (per Google's published limits — there is NO 50K-character cap; the user's 50K figure was a guess). **Automatic rolling strategy (do NOT break the system):** before writing each synced document to the master doc, the backend **anticipates** the resulting size/length — it estimates `current_master_length + incoming_doc_length`; if that sum lands **within 95% of the limit**, it does NOT append — it creates a **new master doc** with the same base name and a numeric suffix (`(MASTER) {title}`, `{title} 2`, `{title} 3`, …) and continues there. Old parts are never deleted and never merged, so the system keeps running regardless. Keep a manifest entry mapping the parts. Frontend Drive Preview lists each part as its own row.
